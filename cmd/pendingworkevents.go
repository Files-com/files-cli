...
func init() {
	RootCmd.AddCommand(PendingWorkEvents())
}

func PendingWorkEvents() *cobra.Command {
	PendingWorkEvents := &cobra.Command{
		Use:   "pending-work-events [command]",
		Short: "A PendingWorkEvent is a log record for pending file work failures, such as GPG Encryption and GPG Decryption.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command pending-work-events\n\t%v", args[0])
		},
	}
	...
	paramsPendingWorkEventList := files_sdk.PendingWorkEventListParams{}
	...
	cmdList := &cobra.Command{
		Use:     "list",
		Short:   "List Pending Work Events",
		Long:    `List Pending Work Events`,
		Args:    cobra.NoArgs,
		Aliases: []string{"ls"},
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			config := ctx.Value("config").(files_sdk.Config)
			params := paramsPendingWorkEventList
			...
			client := pending_work_event.Client{Config: config}
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort pending work events by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find pending work events where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find pending work events where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find pending work events where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find pending work events where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find pending work events where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsPendingWorkEventList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsPendingWorkEventList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	...
	cmdFind := &cobra.Command{
		Use:   "find",
		Short: `Show Pending Work Event`,
		Long:  `Show Pending Work Event`,
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			config := ctx.Value("config").(files_sdk.Config)
			...
			var pendingWorkEvent interface{}
			var err error
			pendingWorkEvent, err = client.Find(paramsPendingWorkEventFind, files_sdk.WithContext(ctx))
			return lib.HandleResponse(ctx, Profile(cmd), pendingWorkEvent, err, Profile(cmd).Current().SetResourceFormat(cmd, formatFind), fieldsFind, usePagerFind, cmd.OutOrStdout(), cmd.ErrOrStderr(), config.Logger)
		},
	}
	cmdFind.Flags().Int64Var(&paramsPendingWorkEventFind.Id, "id", 0, "Pending Work Event ID.")
