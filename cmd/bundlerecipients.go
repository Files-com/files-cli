...
func BundleRecipients() *cobra.Command {
	BundleRecipients := &cobra.Command{
		...
		Short: "A BundleRecipient is a person who has had a bundle shared with them via email invite.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command bundle-recipients\n\t%v", args[0])
		},
	}
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort bundle recipients by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find bundle recipients where field exactly matches value.")
	...
	cmdList.Flags().Int64Var(&paramsBundleRecipientList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	cmdList.Flags().Int64Var(&paramsBundleRecipientList.BundleId, "bundle-id", 0, "List recipients for the bundle with this ID.")
	lib.SetFlagAPIRequired(cmdList.Flags(), "bundle-id")
