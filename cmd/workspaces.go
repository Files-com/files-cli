...
func Workspaces() *cobra.Command {
	Workspaces := &cobra.Command{
		...
		Short: "A Workspace is a lightweight way to organize related resources inside a single Files.com Site.",
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort workspaces by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find workspaces where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find workspaces where field starts with value.")
	...
	cmdList.Flags().Int64Var(&paramsWorkspaceList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
