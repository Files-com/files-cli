...
func init() {
	RootCmd.AddCommand(ApiRequestLogs())
}

func ApiRequestLogs() *cobra.Command {
	ApiRequestLogs := &cobra.Command{
		Use:   "api-request-logs [command]",
		Short: "An ApiRequestLog is an audit log for monitoring and troubleshooting requests made via API to your site.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command api-request-logs\n\t%v", args[0])
		},
	}
	...
	paramsApiRequestLogList := files_sdk.ApiRequestLogListParams{}
	...
	cmdList := &cobra.Command{
		...
		Long:    `List API Request Logs`,
		Args:    cobra.NoArgs,
		Aliases: []string{"ls"},
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			config := ctx.Value("config").(files_sdk.Config)
			params := paramsApiRequestLogList
			...
			client := api_request_log.Client{Config: config}
	...
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find api request logs where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find api request logs where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find api request logs where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find api request logs where field starts with value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-prefix", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find api request logs where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find api request logs where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsApiRequestLogList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsApiRequestLogList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 1000, 1,000 or less is recommended).")
	...
	ApiRequestLogs.AddCommand(cmdList)
