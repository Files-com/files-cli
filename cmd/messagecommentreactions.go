...
func MessageCommentReactions() *cobra.Command {
	MessageCommentReactions := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command message-comment-reactions\n\t%v", args[0])
		},
	}
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort message comment reactions by field in ascending or descending order.")
	...
	cmdList.Flags().Int64Var(&paramsMessageCommentReactionList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	...
	cmdFind := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return lib.HandleResponse(ctx, Profile(cmd), messageCommentReaction, err, Profile(cmd).Current().SetResourceFormat(cmd, formatFind), fieldsFind, usePagerFind, cmd.OutOrStdout(), cmd.ErrOrStderr(), config.Logger)
		},
	}
	...
	cmdCreate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return lib.HandleResponse(ctx, Profile(cmd), messageCommentReaction, err, Profile(cmd).Current().SetResourceFormat(cmd, formatCreate), fieldsCreate, usePagerCreate, cmd.OutOrStdout(), cmd.ErrOrStderr(), config.Logger)
		},
	}
