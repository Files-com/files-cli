...
import (
	...
	"github.com/Files-com/files-sdk-go/v3/priority"
...
func Priorities() *cobra.Command {
	...
	cmdList.Flags().StringVar(&paramsPriorityList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	...
	cmdList.Flags().StringVar(&paramsPriorityList.Action, "action", "", "")
	cmdList.Flags().Int64Var(&paramsPriorityList.Page, "page", 0, "")
