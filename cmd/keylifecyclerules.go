...
func KeyLifecycleRules() *cobra.Command {
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort key lifecycle rules by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find key lifecycle rules where field exactly matches value.")
	...
	cmdList.Flags().Int64Var(&paramsKeyLifecycleRuleList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
