...
func Groups() *cobra.Command {
	...
	cmdUpdate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("workspace-id") {
				lib.FlagUpdate(cmd, "workspace_id", paramsGroupUpdate.WorkspaceId, mapParams)
			}
			if cmd.Flags().Changed("ftp-permission") {
	...
	cmdUpdate.Flags().Int64Var(&paramsGroupUpdate.WorkspaceId, "workspace-id", 0, "Workspace ID")
