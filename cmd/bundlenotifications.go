...
func BundleNotifications() *cobra.Command {
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort bundle notifications by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find bundle notifications where field exactly matches value.")
	...
	cmdList.Flags().Int64Var(&paramsBundleNotificationList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	cmdList.Flags().Int64Var(&paramsBundleNotificationList.BundleId, "bundle-id", 0, "Bundle ID")
