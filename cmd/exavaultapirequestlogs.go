...
import (
	...
	exavault_api_request_log "github.com/Files-com/files-sdk-go/v3/exavaultapirequestlog"
	"github.com/spf13/cobra"
)

func init() {
	RootCmd.AddCommand(ExavaultApiRequestLogs())
}

func ExavaultApiRequestLogs() *cobra.Command {
	ExavaultApiRequestLogs := &cobra.Command{
		Use:   "exavault-api-request-logs [command]",
		Short: "An ExavaultApiRequestLog is an audit log for monitoring and troubleshooting requests to the ExaVault API.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command exavault-api-request-logs\n\t%v", args[0])
		},
	}
	...
	paramsExavaultApiRequestLogList := files_sdk.ExavaultApiRequestLogListParams{}
	...
	cmdList := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			params := paramsExavaultApiRequestLogList
			...
			client := exavault_api_request_log.Client{Config: config}
	...
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find exavault api request logs where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find exavault api request logs where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find exavault api request logs where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find exavault api request logs where field starts with value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-prefix", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find exavault api request logs where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find exavault api request logs where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsExavaultApiRequestLogList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsExavaultApiRequestLogList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 1000, 1,000 or less is recommended).")
	...
	ExavaultApiRequestLogs.AddCommand(cmdList)
	return ExavaultApiRequestLogs
}
