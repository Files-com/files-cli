...
func init() {
	RootCmd.AddCommand(OutboundConnectionLogs())
}

func OutboundConnectionLogs() *cobra.Command {
	OutboundConnectionLogs := &cobra.Command{
		Use:   "outbound-connection-logs [command]",
		Short: "An OutboundConnectionLog is an audit log for monitoring and troubleshooting actions performed by your Remote Servers and Files.com on-premise Agents.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command outbound-connection-logs\n\t%v", args[0])
		},
	}
	...
	paramsOutboundConnectionLogList := files_sdk.OutboundConnectionLogListParams{}
	...
	cmdList := &cobra.Command{
		Use:     "list",
		Short:   "List Outbound Connection Logs",
		Long:    `List Outbound Connection Logs`,
		Args:    cobra.NoArgs,
		Aliases: []string{"ls"},
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			config := ctx.Value("config").(files_sdk.Config)
			params := paramsOutboundConnectionLogList
			...
			client := outbound_connection_log.Client{Config: config}
	...
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find outbound connection logs where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find outbound connection logs where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find outbound connection logs where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find outbound connection logs where field starts with value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-prefix", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find outbound connection logs where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find outbound connection logs where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsOutboundConnectionLogList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsOutboundConnectionLogList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 1000, 1,000 or less is recommended).")
