...
func init() {
	RootCmd.AddCommand(WebDavActionLogs())
}

func WebDavActionLogs() *cobra.Command {
	WebDavActionLogs := &cobra.Command{
		Use:   "web-dav-action-logs [command]",
		Short: "A WebDAVActionLog is an audit log for monitoring and troubleshooting operations made to your site through WebDAV protocol.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command web-dav-action-logs\n\t%v", args[0])
		},
	}
	...
	paramsWebDavActionLogList := files_sdk.WebDavActionLogListParams{}
	...
	cmdList := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			params := paramsWebDavActionLogList
			...
			client := web_dav_action_log.Client{Config: config}
	...
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find web dav action logs where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find web dav action logs where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find web dav action logs where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find web dav action logs where field starts with value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-prefix", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find web dav action logs where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find web dav action logs where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsWebDavActionLogList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsWebDavActionLogList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 1000, 1,000 or less is recommended).")
	...
	WebDavActionLogs.AddCommand(cmdList)
	return WebDavActionLogs
}
