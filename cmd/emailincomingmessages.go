...
import (
	...
	email_incoming_message "github.com/Files-com/files-sdk-go/v3/emailincomingmessage"
	"github.com/spf13/cobra"
)

func init() {
	RootCmd.AddCommand(EmailIncomingMessages())
}

func EmailIncomingMessages() *cobra.Command {
	EmailIncomingMessages := &cobra.Command{
		Use:   "email-incoming-messages [command]",
		Short: "An EmailIncomingMessage is an audit log designed to track status of email to inbox uploads.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command email-incoming-messages\n\t%v", args[0])
		},
	}
	...
	paramsEmailIncomingMessageList := files_sdk.EmailIncomingMessageListParams{}
	...
	cmdList := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			params := paramsEmailIncomingMessageList
			...
			client := email_incoming_message.Client{Config: config}
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort email incoming messages by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find email incoming messages where field exactly matches value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGtArgs, "filter-gt", []string{}, "Find email incoming messages where field is greater than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterGteqArgs, "filter-gteq", []string{}, "Find email incoming messages where field is greater than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-gteq", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterPrefixArgs, "filter-prefix", []string{}, "Find email incoming messages where field starts with value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-prefix", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLtArgs, "filter-lt", []string{}, "Find email incoming messages where field is less than value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lt", "field=value")
	cmdList.Flags().StringArrayVar(&listFilterLteqArgs, "filter-lteq", []string{}, "Find email incoming messages where field is less than or equal to value.")
	lib.SetFlagDisplayType(cmdList.Flags(), "filter-lteq", "field=value")

	cmdList.Flags().StringVar(&paramsEmailIncomingMessageList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsEmailIncomingMessageList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	...
	EmailIncomingMessages.AddCommand(cmdList)
	return EmailIncomingMessages
}
