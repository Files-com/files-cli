...
func Partners() *cobra.Command {
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort partners by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")
	cmdList.Flags().StringArrayVar(&listFilterArgs, "filter", []string{}, "Find partners where field exactly matches value.")
	...
	cmdList.Flags().Int64Var(&paramsPartnerList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
	...
	cmdCreate.Flags().Int64Var(&paramsPartnerCreate.AiAssistantPersonalityId, "ai-assistant-personality-id", 0, "AI Assistant Personality ID assigned to this Partner, if any. Users in the Partner inherit it unless a direct per-user assignment overrides it.")
	...
	cmdUpdate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("ai-assistant-personality-id") {
				lib.FlagUpdate(cmd, "ai_assistant_personality_id", paramsPartnerUpdate.AiAssistantPersonalityId, mapParams)
			}
