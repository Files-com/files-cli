...
func Sites() *cobra.Command {
	...
	cmdUpdate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("email-footer-custom-text") {
				lib.FlagUpdate(cmd, "email_footer_custom_text", paramsSiteUpdate.EmailFooterCustomText, mapParams)
			}
	...
	cmdUpdate.Flags().StringVar(&paramsSiteUpdate.DomainLetsencryptChain, "domain-letsencrypt-chain", "", "Letsencrypt chain to use when registering SSL Certificate for domain. No longer used as of 2026.")
