...
func init() {
	RootCmd.AddCommand(ScimLogs())
}

func ScimLogs() *cobra.Command {
	ScimLogs := &cobra.Command{
		Use:   "scim-logs [command]",
		Short: "A SCIM log entry represents a single SCIM request made to the system.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return clierr.Errorf(clierr.ErrorCodeUsage, "invalid command scim-logs\n\t%v", args[0])
		},
	}
	...
	paramsScimLogList := files_sdk.ScimLogListParams{}
	...
	cmdList.Flags().StringVar(&listSortByArgs, "sort-by", "", "Sort scim logs by field in ascending or descending order.")
	lib.SetFlagDisplayType(cmdList.Flags(), "sort-by", "field=asc|desc")

	cmdList.Flags().StringVar(&paramsScimLogList.Cursor, "cursor", "", "Used for pagination.  When a list request has more records available, cursors are provided in the response headers `X-Files-Cursor-Next` and `X-Files-Cursor-Prev`.  Send one of those cursor value here to resume an existing list from the next available record.  Note: many of our SDKs have iterator methods that will automatically handle cursor-based pagination.")
	cmdList.Flags().Int64Var(&paramsScimLogList.PerPage, "per-page", 0, "Number of records to show per page.  (Max: 10000, 1,000 or less is recommended).")
