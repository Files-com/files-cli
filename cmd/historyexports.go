...
func HistoryExports() *cobra.Command {
	...
	cmdFind := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return lib.HandleResponse(ctx, Profile(cmd), historyExport, err, Profile(cmd).Current().SetResourceFormat(cmd, formatFind), fieldsFind, usePagerFind, cmd.OutOrStdout(), cmd.ErrOrStderr(), config.Logger)
		},
	}
	...
	cmdCreate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return lib.HandleResponse(ctx, Profile(cmd), historyExport, err, Profile(cmd).Current().SetResourceFormat(cmd, formatCreate), fieldsCreate, usePagerCreate, cmd.OutOrStdout(), cmd.ErrOrStderr(), config.Logger)
		},
	}
	...
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryInterface, "query-interface", "", "Filter results by this this interface type. Valid values: `web`, `ftp`, `robot`, `jsapi`, `webdesktopapi`, `sftp`, `dav`, `desktop`, `restapi`, `scim`, `office`, `mobile`, `as2`, `inbound_email`, `remote`, `inbound_s3`")
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryUserId, "query-user-id", "", "Return results that are actions performed by the user indicated by this User ID")
	...
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryPath, "query-path", "", "Return results that are file actions related to paths matching this pattern.")
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryFolder, "query-folder", "", "Return results that are file actions related to files or folders inside folder paths matching this pattern.")
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QuerySrc, "query-src", "", "Return results that are file moves originating from paths matching this pattern.")
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryDestination, "query-destination", "", "Return results that are file moves with paths matching this pattern as destination.")
	...
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryTargetId, "query-target-id", "", "If searching for Histories about specific objects (such as Users, or API Keys), this parameter restricts results to objects that match this ID.")
	...
	cmdCreate.Flags().StringVar(&paramsHistoryExportCreate.QueryTargetPermission, "query-target-permission", "", "If searching for Histories about Permissions, this parameter restricts results to permissions of this level.")
