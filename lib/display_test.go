...
func TestSanitizeArgsForDisplay(t *testing.T) {
	t.Run("it sanitizes api key flags with inline values", func(t *testing.T) {
		...
		require.Equal(t, []string{"files-cli", "--debug=debug.log", "--api-key=0123456789abcdef****************", "version"}, SanitizeArgsForDisplay(args))
	})
	...
	t.Run("it sanitizes api key flags with separate values", func(t *testing.T) {
		...
		require.Equal(t, []string{"files-cli", "config", "set", "--api-key", "0123456789abcdef****************"}, SanitizeArgsForDisplay(args))
	})
	...
	t.Run("it sanitizes short api key flags with separate values", func(t *testing.T) {
		...
		require.Equal(t, []string{"files-cli", "config", "set", "-a", "0123456789abcdef****************"}, SanitizeArgsForDisplay(args))
	})
