...
const (
	serviceName     = "files-cli"
	serviceDesc     = "Files.com Agent"
...
func (s *WindowsService) RunService() error {
	...
	if err != nil {
		return fmt.Errorf("IsWindowsService - %v", err)
	}
	...
	logger.Debug(logSender, "", "RunService - s.isInteractive: %v", s.isInteractive)
...
func (s *WindowsService) Start() error {
	m, err := mgr.Connect()
	if err != nil {
		return fmt.Errorf(" mgr.Connect - %v", err)
	}
	defer m.Disconnect()
